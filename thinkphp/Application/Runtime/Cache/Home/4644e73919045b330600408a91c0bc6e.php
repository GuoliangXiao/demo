...
<div class="row">
	<div class="col-md-8">
		...
		<div class="app-page">
			<?php if(($apppage) > "1"): $__FOR_START_13462__=0;$__FOR_END_13462__=$apppage;for($i=$__FOR_START_13462__;$i < $__FOR_END_13462__;$i+=1){ ?><button class="btn btn-default btn-apppage" id=<?php echo ($i); ?>><?php echo ($i+1); ?></button><?php } endif; ?>
		</div>
	</div>
	<div class="col-md-4">
		<?php $is_phone=is_mobile(); ?>
		<?php if(($is_phone) == "0"): ?><h3><i class="fa fa-thumbs-up fa-1x"></i>&nbsp;点赞排行</h3>
			<ul class="loved-apps">
				<?php if(is_array($loves)): $i = 0; $__LIST__ = $loves;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
						...
						<span>
							<a href="javascript:void(0);"><i id="<?php echo ($vo["id"]); ?>" class="my-heart fa fa-heart fa-1x"></i></a>&nbsp;
...
<script type="text/javascript">
	...
	$(".btn-apppage").click(function(event) {
		...
		var url="<?php echo U('Home/AppList/getApp');?>";
		var e=$(this);
	...
	function showAppData(data){
		$('.my-apps>li').each(function(index, el) {
			if(index<data.length){
				$(this).find('a').attr('href',data[index]['url']);
				$(this).find('img').attr('src',data[index]['icon']);
				$(this).find('span').text(data[index]['name']);
