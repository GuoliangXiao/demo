...
<div class="row">
	<div class="col-md-12">
		<h3 style="text-align: center;"><?php echo ($table_name); ?></h3>
		<table class="table table-striped">
			<tr>
				<?php if(is_array($dbFields)): foreach($dbFields as $key=>$vo): ?><td><?php echo ($vo); ?></td><?php endforeach; endif; ?>
				<td colspan="2">
					<a target="_blank" href="<?php echo U('Admin/EditTable/add?table='.$table_name);?>">
			...
			<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($vo['id']); ?>">
					...
					<td>
						<a href="<?php echo U('Admin/EditTable/edit?id='.$vo['id'].'&table='.$table_name);?>">
...
<script type="text/javascript">
	...
	$(".btn-delete").click(function(event) {
		/* Act on the event */
		var url="<?php echo U('Admin/EditTable/delete');?>";
		var id=$(this).attr('id');
