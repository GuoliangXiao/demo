...
<div class="row">
	<div class="col-md-12">
		...
		<form action="<?php echo U('update?table='.$table.'&id='.$id);?>" method="post">
			<table class="table table-striped">
				<?php if(is_array($dbFields)): foreach($dbFields as $key=>$vo): if(($vo) != "id"): ?><tr class ="<?php echo ($vo); ?>">
							<td class="field"><?php echo ($vo); ?></td>
							<td>
								<input id="<?php echo ($vo); ?>" class="form-control" type="text" name="<?php echo ($vo); ?>" value="<?php echo ($data[0][$vo]); ?>">
							</td>
						</tr><?php endif; endforeach; endif; ?>
			</table>
			<button style="width: 100%;" class="btn btn-info" type="submit">更新</button>			
		</form>
	</div>
</div>
